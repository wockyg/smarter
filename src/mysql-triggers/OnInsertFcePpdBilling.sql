CREATE TRIGGER `OnInsertFcePpdBilling` BEFORE INSERT ON `fceppdBilling` FOR EACH ROW BEGIN 
	SET @service = (SELECT service FROM define20_smarterbeta2.referralsNotification WHERE referralId = NEW.referralId);
	IF @service = "FCE" THEN
		SET @fceRate = (
			SELECT fceRate
			FROM define20_smarterbeta2.adjusters a
		...
		SET @facilityRate = (
			SELECT fceRate
			FROM define20_smarterbeta2.therapists t
