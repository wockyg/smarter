...
export default function ReferralsByMonthLineChart(props) {
    ...
    const referralsFiltered = referrals?.filter(r => new Date(r.referralDate).getFullYear() === +year);
                                        // .map(r => {
                                        //         const referralMonth = new Date(r.referralDate).getMonth() + 1;
                                        //         const referralYear = new Date(r.referralDate).getFullYear();
                                        //         return ({...r, referralMonth: referralMonth, referralYear: referralYear})
                                        //     });
    ...
    // referrals && console.log(referralsPivot);
    ...
    return(
        <Box sx={{background: '#FFFFFF', marginLeft: 2}}>
            ...
            <Box sx={{background: '#ECF0F2'}}>
                ...
                yAxis={[
                    { 
                        min: Math.min(...referralsPivot?.map(r => r.numReferrals)) - 20,
                        max: Math.max(...referralsPivot?.map(r => r.numReferrals)) + 20
                    },
                ]}
                series={[
                    {
                        dataKey: 'numReferrals',
                    },
                ]}
                width={600}
                height={300}
                dataset={referralsPivot}
                />
            </Box>
