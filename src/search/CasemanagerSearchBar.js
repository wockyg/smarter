...
export default function CasemanagerSearchBar(props) {
    ...
    const handleClearSearch = () => {
        setSearchInputVals({...searchInputVals, casemanagerClient: ''})
        setSearchVals({...searchVals, casemanager: '', casemanagerStatus: '', casemanagerClient: null});
        setDetailsId({...detailsId, 'casemanager': -1})
    };

    const handleChangeStatus = (e) => {
        setSearchVals({...searchVals, casemanagerStatus: e.target.value});
    };
      ...
      <Box sx={{borderRadius: 1, padding: 1, background: '#D5D8DC', width: '100%'}}>
        <Grid container spacing={1}>
          ...
          <Grid item>
            ...
            onChange={(e) => setSearchVals({...searchVals, casemanager: e.target.value})}
            value={searchVals.casemanager}
            label="Name"
            InputProps={{
                endAdornment: (
                <IconButton 
                onClick={() => setSearchVals({...searchVals, casemanager: ''})}
                sx={{visibility: searchVals.casemanager !== '' ? 'visible' : 'hidden'}}
                >
                    <HighlightOffIcon />
                </IconButton>
                )
          ...
          <Grid item>
            ...
            value={searchVals.casemanagerClient}
            onChange={(event, claim) => {
              setSearchVals({...searchVals, casemanagerClient: claim})
            }}
            inputValue={searchInputVals.casemanagerClient}
            onInputChange={(event, newInputValue) => {
              setSearchInputVals({...searchInputVals, casemanagerClient: newInputValue});
            }}
          ...
          <Grid item>
            ...
            <FormControl fullWidth>
              ...
              value={searchVals.casemanagerStatus}
              label="Status"
